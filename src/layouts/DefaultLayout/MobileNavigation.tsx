...
const icons: MobileNavItem[] = [
    ...
    {
        path: "/courses",
        matches: ["/courses", "/buy-course", "/course-purchase"],
        ariaLabel: "Kurslar",
    ...
    {
        path: "/my-courses",
        matches: ["/my-courses", "/test-history", "/quiz-history", "/homework-history"],
        ariaLabel: "Mening kurslarim",
    ...
    return (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-md lg:hidden z-[100] animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="rounded-[32px] backdrop-blur-xl bg-white/80 dark:bg-slate-900/90 flex items-center justify-between p-3 px-6 border border-white/40 dark:border-white/10 shadow-[0_8px_32px_-4px_rgba(0,0,0,0.1)] dark:shadow-[0_8px_32px_-4px_rgba(0,0,0,0.5)]">
                {icons.map((icon) => {
                    const matchers = icon.matches ?? [icon.path];
                    ...
                    if (isActive) {
                        return (
                            <div
                                ...
                                className="relative flex items-center justify-center p-2 rounded-2xl bg-blue-50 dark:bg-blue-900/30 transition-all duration-300"
                            >
                                {icon.activeIcon}
